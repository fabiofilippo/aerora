...
import it.exolab.aero.service.controllers.OtpService;
...
@RestController
@RequestMapping(RestConstants.Airport_01Path.OTP)
@CrossOrigin
...
//    }
    ...
    public ResponseEntity<ResponseDto> recuperoPass(@RequestBody CustomerDto customerDto) {
        ResponseDto responseDto = new ResponseDto();
        try {
            OtpDto otpDto = otpService.recuperoPass(customerDto);
            responseDto.setData(otpDto);
    ...
    @PostMapping("/otp")
    public ResponseEntity<ResponseDto> verificaOtp(@RequestBody OtpDto otpDto) {
        ResponseDto responseDto = new ResponseDto();
        try {
            otpDto = otpService.verificaOtp(otpDto);
            responseDto.setData(otpDto);
            return new ResponseEntity<>(responseDto, HttpStatus.OK);
        } catch (Exception e) {
            e.printStackTrace();
